        ...
        for (unsigned int point=0; point<fe_v.n_quadrature_points; ++point)
          {
            ...
            if (component_i == EulerEquations<dim>::first_momentum_component+1)
            {
            	R_i -= ( parameters.theta * W[point][component_i+1]*gravity +
                        (1.0-parameters.theta) * W_old[point][component_i+1]*gravity ) *
                        		fe_v.shape_value_component(i, point, component_i) *
                      fe_v.JxW(point);
            }
          }
